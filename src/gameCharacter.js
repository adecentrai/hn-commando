/*
    HN Commando - Character
    - Platform style character shared by the player and enemy soldiers
    - Characters can jump, shoot, roll, throw grenades and climb ladders
    - Has a weapon, health, a team and spawn protection
...
export class Character extends GameObjects.GameObject
{
    constructor(pos)
    {
        ...
        this.bloodColor = hsl(0,1,.5);
        this.renderOrder = 10;
        this.walkCyclePercent = 0;
        this.walkSoundTime = 0;
        this.health = 1;
        this.maxSpeed = .2;
        this.spawnProtection = 1;  // seconds of invulnerability after spawning
    ...
    update()
    {
        ...
        else
        {
            ...
            if (this.pressingThrow && !this.wasPressingThrow && !this.grenadeThrowTimer.active() && this.grenadeCount > 0)
            {
                // throw grenade
        ...
        this.velocity.x = LJS.clamp(this.velocity.x + moveInput.x * .04, -this.maxSpeed, this.maxSpeed);
    ...
    isProtected() { return this.getAliveTime() < this.spawnProtection; }

    render()
    {
        // flicker see-through while spawn protected
        let color = this.color;
        if (!this.isDead() && this.isProtected() && (LJS.time*10|0)%2)
        ...
        LJS.drawTile(bodyPos, this.drawSize, this.tileInfo, color, this.angle, this.mirror, this.additiveColor);
    }

    damage(damage, damagingObject)
    {
        if (this.isDead() || this.isProtected() || this.dodgeTimer.active())
            return 0;

        GameEffects.makeBlood(damagingObject ? damagingObject.pos : this.pos, 50, this.bloodColor);
    ...
    kill(damagingObject)
    {
        ...
        GameEffects.makeBlood(this.pos, 100, this.bloodColor);
